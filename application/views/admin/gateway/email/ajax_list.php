...
foreach ($list as $gateway) {
    ...
    $row[] = '<a class="btn btn-primary btn-sm" href="javascript:;" onclick="popup_add_mgateway(' . $gateway->id . ');">Edit</a> &nbsp'
            . ' <a class="btn btn-danger btn-sm" href="javascript:;" onclick="delete_mgateway(' . $gateway->id . ');">Delete</a>';
    $data[] = $row;
}
