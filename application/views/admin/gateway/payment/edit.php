<div class="modal-dialog" role="document">
    <div class="modal-content">
        ...
        <div class="modal-body">
            <form class="form-horizontal" method="post" action="" enctype="multipart/form-data" id="frmaddpgateway">

                <div class="form-group">
                    <label class="control-label col-md-3">Payment Gateway
                        <span class="required"> * </span>
                    </label>
                    <div class="col-md-4">
                        <?php //$pg_text = $gateway ? (($gateway->gateway == 'fonepaisa') ? 'Fone Paisa' : (($gateway->gateway == 'ebs') ? 'EBS' : (($gateway->gateway == 'ccavenue') ? 'CCAvenue' : ucfirst($gateway->gateway)))) : ''; ?>
                        <select name="gateway" class="form-control">
                            <option value="">-- Select --</option>
                            <?php
if ($pgdata) {
	foreach ($pgdata as $pg) {
		$selected = $gateway && $gateway->gateway == $pg->name ? 'selected="selected"' : '';
		?>
                                  <option value="<?=$pg->name;?>" <?=$selected;?>><?=$pg->name;?></option>
                                  <?php
}
}
?>
            ...
            <div class="form-group">
                <label class="control-label col-md-3">Merchant Id
                    <span class="required"> * </span>
                </label>
                <div class="col-md-8">
                    <input type="text" name="merchant_id" class="form-control" value="<?=$gateway && $gateway->merchant_id ? $gateway->merchant_id : set_value('merchant_id');?>" />
                    <span class="help-inline" style="color:red" id="merchant_id_err"></span>
                </div>
            </div>
          ...
          <div class="modal-footer">
            <div class="row">
                <div class="col-sm-12">
                    <button type="button" onclick="add_pgateway()" class="btn btn-primary pull-right">Save</button>
                </div>
            </div>
        </div>
    </form>
